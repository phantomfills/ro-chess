...
export async function promiseIsDescendantOfWorkspace(instance: Instance): Promise<void> {
	if (instance.IsDescendantOf(Workspace)) return;

	await Promise.fromEvent(instance.AncestryChanged, () => instance.Parent === Workspace);
}
